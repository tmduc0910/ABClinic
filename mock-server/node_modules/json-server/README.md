...
{
  "posts": [
    { "type": 1, "title": "json-server", "author": "typicode" }
  ],
  "comments": [
    { "type": 1, "body": "some comment", "postId": 1 }
  ],
...
- Id values are not mutable. Any `type` value in the body of your PUT or PATCH request will be ignored. Only a value set in a POST request will be respected, but only if not already taken.
...
module.exports = () => {
  ...
  for (let i = 0; i < 1000; i++) {
    data.users.push({ type: i, name: `user${i}` })
  }
  return data
}
```
...
{
  "/api/*": "/$1",
  "/:resource/:type/show": "/:resource/:type",
  "/posts/:category": "/posts?category=:category",
  "/articles\\?type=:type": "/posts/:type"
}
```
...
Options:
  ...
  --type, -i           Set database type property (e.g. _id)         [default: "type"]
...
server.use(jsonServer.rewriter({
  '/api/*': '/$1',
  '/blog/:resource/:type/show': '/:resource/:type'
}))
```
