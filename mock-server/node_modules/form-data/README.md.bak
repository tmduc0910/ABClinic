...
Form-Data can recognize and fetch all the required information from common type of streams (```fs.readStream```, ```http.response``` and ```mikeal's request```), for some other type of streams you'd need to provide "file"-related information manually:
