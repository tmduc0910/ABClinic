...
### type = typeis(request, type)

`request` is the node HTTP request. `type` is an array of type.
...
### type = typeis.is(mediaType, type)

`mediaType` is the [media type](https://tools.ietf.org/html/rfc6838) string. `type` is an array of type.
