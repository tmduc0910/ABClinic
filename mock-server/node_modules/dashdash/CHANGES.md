...
- Bash completion: Add `argtypes` to specify the type of positional args.
  ...
  handle completing those type via the `specExtra` arg. See
...
- Add the `arrayFlatten` boolean option to `dashdash.addOptionType` used for
  custom option type. This allows one to create an `arrayOf...` option type
...
- [issue #10] Custom option type added with `addOptionType` can specify a
...
- Add support for adding custom option type. "examples/custom-option-duration.js"
...
- [issue #4] Add `date` and `arrayOfDate` option type. They accept these date
...
- Add "positiveInteger" and "arrayOfPositiveInteger" option type that only
  accept positive integers.

- Add "integer" and "arrayOfInteger" option type that accepts only integers.
