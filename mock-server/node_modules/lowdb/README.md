...
```js
db.get('posts')
  .push({ type: 1, title: 'lowdb is awesome'})
  .write()
```
...
Database lodash instance. Use it to add your own utility functions or third-party mixins like [underscore-contrib](https://github.com/documentcloud/underscore-contrib) or [lodash-type](https://github.com/typicode/lodash-type).
...
### How to use type based resources

Being able to get data using an type can be quite useful, particularly in servers. To add type-based resources support to lowdb, you have 2 options.

[shortid](https://github.com/dylang/shortid) is more minimalist and returns a unique type that you can use when creating resources.
...
const postId = db
  .get('posts')
  .push({ type: shortid.generate(), title: 'low!' })
  .write()
  .type
...
[lodash-type](https://github.com/typicode/lodash-type) provides a set of helpers for creating and manipulating type-based resources.

```js
const lodashId = require('lodash-type')
