...
{{? it.isTop }}
  var validate = {{?$async}}{{it.async = true;}}async {{?}}function(data, dataPath, parentData, parentDataProperty, rootData) {
    'use strict';
    {{? $type && (it.opts.sourceCode || it.opts.processCode) }}
      {{= '/\*# sourceURL=' + $type + ' */' }}
    {{?}}
{{?}}
...
{{??}}
  {{
    ...
    if ($type) it.baseId = it.resolve.url(it.baseId, $type);
...
{{? $typeSchema }}
  ...
  {{ var $rulesGroup = it.RULES.type[$typeSchema]; }}
